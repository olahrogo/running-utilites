...
export function calculateElevationGainForSegments(trackPoints: TrackPoint[], segmentDistanceMeters: number = 1000): ElevationAnalysisResult {
  ...
  // Process each track point to calculate distances and elevation changes
  for (let i = 1; i < trackPoints.length; i++) {
    ...
    if (totalDistance >= currentSegment * segmentDistanceMeters || i === trackPoints.length - 1) {
      // Calculate elevation change for this segment
      let elevationGain = 0;
      let elevationLoss = 0;
      
      for (let j = currentSegmentStartIdx + 1; j <= i; j++) {
        const eleDiff = trackPoints[j].ele - trackPoints[j - 1].ele;
        if (eleDiff > 0) {
      ...
      // Calculate segment distance in km and grade
      const segmentDistanceInMeters = (totalDistance - (currentSegmentStartIdx > 0 ? 
        (currentSegment - 1) * segmentDistanceMeters : 0));
      ...
      const segmentEnd = i === trackPoints.length - 1 ? 
      ...
      segmentResults.push({
        ...
        current_elevation: Math.round(trackPoints[i].ele * 10) / 10
      });
  ...
  const totalElevationGain = segmentResults.reduce((sum, km) => sum + km.elevation_gain, 0);
  const totalElevationLoss = segmentResults.reduce((sum, km) => sum + km.elevation_loss, 0);
  const netElevation = totalElevationGain - totalElevationLoss;
  const overallGrade = (netElevation / totalDistance) * 100;
  
  // Calculate average elevation gain normalized to km
  const kmFactor = 1000 / segmentDistanceMeters;
  const avgElevationGainPerKm = (totalElevationGain / segmentResults.length) * kmFactor;
  ...
  return {
    ...
    start_elevation: Math.round(trackPoints[0].ele * 10) / 10,
    end_elevation: Math.round(trackPoints[trackPoints.length - 1].ele * 10) / 10
  };
}

// Main function to analyze GPX data
export function analyzeGpxElevation(gpxString: string, filename?: string, segmentDistanceMeters: number = 1000): ElevationAnalysisResult {
  try {
    ...
    const results = calculateElevationGainForSegments(trackPoints, segmentDistanceMeters);
